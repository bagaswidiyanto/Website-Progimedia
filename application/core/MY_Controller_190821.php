		...
		private function menu($parent=0,$hasil){
		 $w = $this->CI->db->query("SELECT * from tbl_navigation where parent=".$parent." and topbar=1 order by `sort` asc");
			...
			foreach($w->result() as $h)
			{
				...
				// $hasil .= "<li class='dropdown' style='margin-left: 15px;margin-right: 15px;'><a data-toggle='dropdown' href='".base_url($slug)."' style='margin-left: 5px;'><img class='' alt='' src='".base_url()."assets/img/".$h->image."' style='width:23px;height:23px;margin-top: -25px;margin-left:10px;'><p style='margin-bottom: -20px;margin-top: -30px;'>".$h->title."</p><span class='caret'></span></a>";

				$hasil .= "<li class='dropdown'><a data-toggle='dropdown' href='#".$judul."' style='margin-left: 5px;'><p>".$h->title."</p><span class='caret'></span></a>";
				}else{
				// $hasil .= "<li class='dropdown ' ><a  href='".base_url($slug)."' style='margin-left: 5px;'><img class='' alt='' src='".base_url()."assets/img/".$h->image."' style='width:23px;height:23px;margin-top: -25px;margin-left:10px;'><p style='margin-bottom: -20px;margin-top: -30px;'>".$h->title."</p></a>";

				$hasil .= "<li class='dropdown-".$h->sort."' ><a  href='#".$judul."' style='margin-left: 5px;'><p>".$h->title."</p></a>";	
				}
